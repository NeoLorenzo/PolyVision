...
The active genuine pools are 11×11. The current environment derives a 6,424-value observation (under `v5_human_information_parity`) and 63,913-ID global action catalog from that geometry, and checkpoint sidecars enforce geometry/interface compatibility. The wrapper defaults to `POLYVISION_LEVEL_POOL_GLOB=levels/phase1_pool_bardur_real/train/*.csv`; evaluation must select another canonical pool explicitly.
