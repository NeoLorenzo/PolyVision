...
func DeserializePlayerDataFromBytes(streamReader *io.SectionReader, gameVersion int) PlayerData {
	...
	// Version 122 may prefix a player name with an observed 0xff marker. Peek
	// and consume it without assigning semantics; this also remains compatible
	// with player records that begin directly with the name length.
	if gameVersion >= 122 {
		markerOffset, _ := streamReader.Seek(0, io.SeekCurrent)
		if unsafeReadUint8(streamReader) != 0xff {
			_, _ = streamReader.Seek(markerOffset, io.SeekStart)
		}
	}
