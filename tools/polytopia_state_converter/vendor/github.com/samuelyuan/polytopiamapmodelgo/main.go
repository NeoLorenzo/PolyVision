...
// ReadPolytopiaCompressedInitialFile parses only the generated initial state.
// PolyVision intentionally does not parse or depend on the later/current map,
// players, or actions when producing its canonical generated-map dataset.
func ReadPolytopiaCompressedInitialFile(inputFilename string) (*PolytopiaSaveOutput, error) {
	decompressedReader, decompressedLength := BuildReaderForDecompressedFile(inputFilename)
	streamReader := io.NewSectionReader(decompressedReader, 0, int64(decompressedLength))

	fileOffsetMap = make(map[string]int)
	initialHeader := DeserializeMapHeaderFromBytes(streamReader)
	if initialHeader.MapWidth <= 0 || initialHeader.MapHeight <= 0 {
		return nil, fmt.Errorf("invalid initial map dimensions %dx%d", initialHeader.MapWidth, initialHeader.MapHeight)
	}
	initialTiles := make([][]TileData, initialHeader.MapHeight)
	for y := range initialTiles {
		initialTiles[y] = make([]TileData, initialHeader.MapWidth)
	}
	gameVersion := int(initialHeader.MapHeaderInput.Version1)
	readTileData(streamReader, initialTiles, initialHeader.MapWidth, initialHeader.MapHeight, gameVersion)
	initialPlayers := readAllPlayerData(streamReader, gameVersion)

	return &PolytopiaSaveOutput{
		MapHeight:         initialHeader.MapHeight,
		MapWidth:          initialHeader.MapWidth,
		GameVersion:       gameVersion,
		MapHeaderOutput:   initialHeader,
		InitialTileData:   initialTiles,
		InitialPlayerData: initialPlayers,
		FileOffsetMap:     fileOffsetMap,
		OwnerTribeMap:     buildOwnerTribeMap(initialPlayers),
	}, nil
}
