...
func DeserializeMapHeaderFromBytes(streamReader *io.SectionReader) MapHeaderOutput {
	...
	// Polytopia version 122 introduced one header byte before map dimensions.
	// It is currently zero in harvested saves but must be consumed to preserve
	// alignment. Retain raw IDs elsewhere rather than interpreting this byte.
	if mapHeaderInput.Version1 >= 122 {
