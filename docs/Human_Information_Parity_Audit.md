...
| **Unit Current & Max HP** | Health bar / number over unit | Yes (`Unit.currentHP`, `maxHP`) | Yes (`unit.currentHP`) | Deterministic via Unit Type (H1/H2) | Full | `PARITY` | `STATIC_RULE_KNOWLEDGE` | `Unit.java:51-53`, `TribesConfig.java:48,75,94,103,145`, `PythonEnv.java:375` | Health bar rendered above unit. Current/max HP are not globally constant at 10/10; unit max HP is a deterministic static function of the visible unit type (10 for Warrior/Archer/Catapult/Rider/MindBender, 15 for Defender/Swordsman/Knight, 40 for SuperUnit per TribesConfig). Because ATTACK/combat is excluded in Phase 1, reachable units take 0 damage, so current HP remains equal to their type-specific max HP. No two reachable Phase-1 states can share an identical unit-type configuration while differing only in HP; information is exactly recoverable from visible unit type + public static engine rules. |
| **Veteran Status & Kills** | Crown badge / kill count | Yes (`Unit.isVeteran`, `kills`) | Yes (`unit.isVeteran`, `kill`) | Invariant (0 kills, non-veteran) | Full | `PARITY` | `DORMANT_PHASE1` | `Unit.java:60-71`, `PythonEnv.java:371` | In Phase 1 solo mode, units cannot fight or earn kills; ruin-spawned units spawn with 0 kills and non-veteran. Strictly invariant in Phase 1. |
| **Home City ID** | Selecting unit highlights home city | Yes (`Unit.cityId`) | Yes (`unit.cityID`) | Yes (9 binary spatial channels `unit_home_city_slot_0..8`, fog-masked) | Full 2D map | `PARITY` | — | `Unit.java:79`, `Board.java:859-863`, `ExamineCommand.java:52`, `PythonEnv.java:373`, `register_env.py:5178-5190` | Selecting unit highlights home city. Direct 9-channel spatial mapping (`unit_home_city_slot_0..8`) from unit position to deterministic home-city slot (Resolved PARITY-002.1). `cityID == -1` is the legitimate engine representation for an extra-unit state (e.g. ruin-spawned SuperUnit belonging directly to the tribe rather than a supporting city), represented as unit present in unit-type channels with all 9 home-city slot channels zero (Resolved PARITY-002.2). |
...
| **Unit Home-City Association** | Yes (9 planes = 1,089) | Implicit | Implicit | **YES** | Yes | `PARITY` | 9-channel spatial mapping to deterministic city slot (Resolved PARITY-002.1). Legitimate `cityID == -1` extra units encoded with all 9 home-city slot channels zero (Resolved PARITY-002.2). |
| **Unit Current & Max HP** | Deterministic via Unit Type (H1/H2) | Implicit | Implicit | **YES** | Yes | `PARITY` | Max HP is a deterministic static function of the visible unit type (10, 15, 40 HP per TribesConfig). With ATTACK excluded in Phase 1, reachable units take no damage (current HP == max HP). Exactly recoverable from unit type + public static rules. |
...
| `STATE-UNIT-002` | State | Unit Current & Max HP | Health bar over unit | Deterministic via unit type + static rules (current HP == max HP) | `PARITY` | **Low (Solo)** | **Critical** | **PARITY / STATIC RULE (Phase 1)** |
| `STATE-UNIT-003` | State | Unit Home-City Association | Pulse city dot on unit select | 9 unit home-city spatial channels (all 0 for cityID=-1) | `PARITY` | **Medium** | **High** | **RESOLVED (PARITY-002.2)** |
...
| **P3** | Unit Home-City Association Channels (`STATE-UNIT-003`) | **CLOSED** | PARITY-002.1 / PARITY-002.2 |
| **P4** | Multi-Agent Unit Combat HP / Veteran (`STATE-UNIT-002`) | **PARITY (Static Rules)** | Deferred for Full-Game Combat |
...
- **PARITY-002.2 (Ruin SuperUnit No-Home-City Parity Hotfix):**  
  Hardened `STATE-UNIT-003` unit home-city encoding semantics to legitimately accept `cityID == -1` (extra units belonging directly to the tribe, such as ruin-spawned SuperUnits) by representing them as unit present in unit-type channels with all 9 home-city channels set to 0.0, while continuing to fail closed on unmapped positive city IDs. Reconciled `STATE-UNIT-002` unit HP parity as a deterministic static function of unit type rather than an invariant 10/10 claim.
