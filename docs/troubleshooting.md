...
On the current corpus under `v4_exact_per_city_state`, expected values are observation shape `(586,)`, action space `63913`, legal capacity `256`, and action-feature dimension `42`. A fallback warning followed by placeholder dimensions means initialization failed earlier; fix the preceding JVM/map error rather than training with the placeholders.
